...
class ForumListViewModel extends ChangeNotifier {
  ...
  List<ForumViewModel> ownForums = <ForumViewModel>[];
  ...
  Future<void> getOwnAllForums() async {
    ...
    ownForums = results.map((forum) => ForumViewModel(forum: forum)).toList();
    status = ownForums.isEmpty ? Status.empty : Status.success;
