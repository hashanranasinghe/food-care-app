...
class FoodApiServices {
  ...
  static Future<int> createFoodPost({required Food food}) async {
    ...
    try {
      ...
      request.fields['isComplete'] = food.isComplete.toString();
  ...
  static Future<int> updateIsCompleteStatus(
      {required String foodId, required bool isComplete}) async {
    print(foodId);
    int res = resFail;
    String? token = await StoreToken.getToken();
    try {
      final comData = jsonEncode({'isComplete': isComplete.toString()});
      final response = await client.put(
          Uri.http(Config.apiURL, Config.completeFood(id: foodId)),
          headers: {
            'Authorization': 'Bearer $token',
      ...
      if (response.statusCode == 200) {
        res = resOk;
        print('isComplete status updated.');
        return res;
      } else {
        print('Failed to update isComplete status.');
        return res;
      }
    } catch (error) {
      print('Error: $error');
  ...
  static Future<int> requestFoodPost(
    ...
    final userData = jsonEncode({'requesterId': requesterId});
    final response = await client.put(
  ...
  static Future<int> permissionFoodPost(
    ...
    // Send a PUT request to the API with the token in the Authorization header
    final userData =
        jsonEncode({'foodId': foodId, 'userId': requesterId, 'status': status});
  ...
  static Future<List<Food>> getRequestFoodPosts(String path) async {
    String? token = await StoreToken.getToken();
    final response = await client.get(
...
print(jsonList.length);
  ...
  static Future<List<Food>> getAcceptFoodPosts(String path) async {
    String? token = await StoreToken.getToken();
    final response = await client.get(
      Uri.http(Config.apiURL, path),
      headers: {
  ...
  static Future<List<Food>> getCompleteFoodPosts(String path) async {
    String? token = await StoreToken.getToken();
    final response = await client.get(
      Uri.http(Config.apiURL, path),
      headers: {
